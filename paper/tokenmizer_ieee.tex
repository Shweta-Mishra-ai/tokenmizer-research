...
An initial pass of this study measured TokenMizer at $57\%$
macro F1, trailing both Graphiti-style and Mem0-style by a
small but consistent margin. Error analysis on the same
per-session results traced the two weakest categories ---
decisions and errors --- to four specific, fixable causes in
the extraction code (Section~\ref{sec:fix-cycle}); after
fixing them and re-running the identical benchmark, TokenMizer
(now version 0.5.3) scores $\mathbf{60\%}$ macro F1
($95\%$ CI $[55\%,65\%]$), significantly higher than
GraphRAG-style ($44\%$), MemGPT-style ($35\%$), and all
three naive baselines ($17$--$20\%$), and
\emph{statistically indistinguishable} from the Graphiti-style
($59\%$) and Mem0-style ($60\%$) reimplementations it
previously trailed (paired bootstrap $95\%$ CI on the
difference now spans $[-3,+4]$ points for both, centred
...
compactly (Section~\ref{sec:extended-methods}).
Its two weakest categories are still decision extraction
($59\%$ F1, up from $50\%$) and error extraction ($44\%$ F1,
up from $36\%$) --- improved, not solved, by the fix cycle.
Every pattern-matching method tested, TokenMizer included,
still degrades to $19$--$27\%$ macro F1 on sessions that state
facts indirectly rather than with explicit lexical markers,
essentially unchanged by the fix cycle --- a shared ceiling of
regex-based extraction that this study is, to our knowledge,
the first to quantify across multiple independently-implemented
memory strategies on one corpus, and that a vocabulary fix
does not move.
...
\textbf{TokenMizer} is the unmodified product package, invoked
out-of-process against a real checkout so that its
\texttt{tokenmizer} module cannot be shadowed by any
...
(Section~\ref{sec:llm-extractor}) is not exercised. The headline
numbers in this section are version 0.5.3; Section~\ref{sec:fix-cycle}
reports the 0.5.2 numbers this study originally measured, the four
causes traced from them, and the fixes that produced 0.5.3, as a
single fix-and-remeasure cycle rather than two disconnected results.
...
the headline comparison for TokenMizer 0.5.3, after the fix
cycle of Section~\ref{sec:fix-cycle}; the 0.5.2 numbers this
study originally measured are Table~\ref{tab:fix-cycle}'s
``before'' column.
...
the same conclusion as the 0.5.2 measurement, but from a point
estimate that now leads rather than trails.
TokenMizer's resume block (100 tokens) is smaller than
Graphiti-style's and Mem0-style's (117--119) but larger than
...
\begin{table}[t]
  \centering
  \caption{Overall Results, n=100, TokenMizer 0.5.3 (100
  ...
  \begin{tabular}{@{}lrrrr@{}}
    ...
    \midrule
    \rowcolor{rowours}
    \textbf{TokenMizer 0.5.3} & \textbf{60\%} & [55, 65] & 100 & 43.8 \\
    Mem0-style       & 60\% & [55, 64] & 117 & 0.9 \\
    Graphiti-style   & 59\% & [55, 64] & 119 & 1.0 \\
    GraphRAG-style   & 44\% & [41, 48] & 80  & 0.5 \\
    MemGPT-style     & 35\% & [32, 38] & \textbf{60}  & 0.4 \\
...
\begin{table}[t]
  \centering
  \caption{Paired Bootstrap: TokenMizer 0.5.3 $-$ Method,
...
\begin{figure}[t]
  ...
  \caption{Paired bootstrap difference (TokenMizer 0.5.3 $-$
    method) on macro F1, n=100, 95\% CI. Intervals excluding
    zero (black) are reliable differences; the two grey
    intervals (Graphiti-style, Mem0-style) cross zero. All five
    black intervals sit further from zero than in the 0.5.2
    measurement (Table~\ref{tab:fix-cycle}), and the two grey
    ones have moved from entirely negative to centred on zero.}
  \label{fig:paired}
\end{figure}
...
the headline score by category, for TokenMizer 0.5.3.
TokenMizer's file extraction is effectively solved ($98\%$ F1,
the best of any method) and its completed-task recall leads
the comparison group. Its two weakest categories are still
\textbf{decisions} ($59\%$, against $65\%$ for both
Graphiti-style and Mem0-style) and \textbf{errors} ($44\%$,
against $66\%$ for the same two) --- narrower gaps than the
0.5.2 measurement's 15 and 30 points, but not closed. Of 148
labelled errors, 0.5.3 extracts 93 (recall $36\%$, up from
$28\%$), of which 41 do not match any ground-truth item
(precision $56\%$); the remaining gap to Graphiti/Mem0-style
is now a recall gap more than a precision gap, which
Section~\ref{sec:fix-cycle} returns to.

\begin{table}[t]
  \centering
  \caption{Per-Category Micro F1, n=100, TokenMizer 0.5.3.}
  \label{tab:category}
  \renewcommand{\arraystretch}{1.15}
...
macro F1 by register, for TokenMizer 0.5.3.
...
47-point drop, both figures slightly higher than the 0.5.2
measurement ($71\%$, $25\%$) but the drop itself essentially
unchanged; Graphiti-style and Mem0-style fall from $85\%$
...
of it, and Section~\ref{sec:fix-cycle} shows directly that
vocabulary and context fixes able to move the explicit- and
semi-register scores measurably do not move it --- consistent
with none of the four methods' extraction path in this study
involving a language model that could plausibly reach implicit
phrasing (Section~\ref{sec:llm-extractor}).

\begin{table}[t]
  \centering
  \caption{Macro F1 by Register, n=100, TokenMizer 0.5.3.}
  \label{tab:register}
  \renewcommand{\arraystretch}{1.15}
  \begin{tabular}{@{}lrrrr@{}}
    \toprule
...
\begin{figure}[t]
  ...
  \caption{Macro F1 by register, n=100, TokenMizer 0.5.3. All
    four pattern-matching methods converge to 19--27\% once
    explicit markers (``Decided:'', ``Completed:'') are absent
    from the text, a shared ceiling of the extraction strategy
...
$58\%$ on the 94 generated sessions --- the real-session score
unchanged by the fix cycle, the synthetic score up 3 points
with it; Graphiti-style and Mem0-style show the opposite,
near-flat pattern ($58\%$ vs.\ $60\%$).
...
TokenMizer's mean 43.8\,ms per session is 45--120$\times$ the
reimplemented methods' 0.4--1.0\,ms.
...
asymptotic cost; the two additional decision/error regex
passes added in Section~\ref{sec:fix-cycle} account for a small
part of the increase from 0.5.2's 38.0\,ms, not the process
overhead, which dominates both measurements.

% ================================================================
\section{Closing the Gap: A Fix-and-Remeasure Cycle}
\label{sec:fix-cycle}
% ================================================================

Sections~\ref{sec:extended-scoring}--\ref{sec:extended-register}
report TokenMizer 0.5.3. This section reports what 0.5.2
originally measured, the four causes traced from it, one fix
that was tried and rejected, and the fixes that produced 0.5.3
--- a complete cycle, not two disconnected numbers.

\subsection{What 0.5.2 Measured}

The first pass of this study measured TokenMizer 0.5.2 at
$57\%$ macro F1 ($95\%$ CI $[52\%,62\%]$), trailing both
Graphiti-style ($59\%$) and Mem0-style ($60\%$) by a small
margin, with decision F1 at $50\%$ and error F1 at $36\%$ ---
both the lowest of any category and both behind the same two
comparison methods by 15 and 30 points respectively
(Section~\ref{sec:experiments}, Table~\ref{tab:pilot-baselines}
reports the unrelated $n=21$ pilot numbers for contrast). These
two categories, being simultaneously TokenMizer's weakest in
absolute terms \emph{and} behind peers scored on the identical
corpus, were the concrete target Discussion point three of the
0.5.2 analysis identified.

\subsection{Tracing Four Causes}

Every missed and spurious decision and error TokenMizer 0.5.2
produced across all 100 sessions is in the released per-session
CSV; reading the actual text behind each one, rather than only
the aggregate score, surfaced four distinct, fixable causes ---
not a diffuse "needs more data" gap:

\begin{enumerate}
\item \textbf{False decisions from questions.} A user follow-up
  like ``What did you decide on the approach?'' matched the
  decision extractor's verb list (\emph{decide}) exactly like a
  real decision statement, extracting ``the approach'' as a
  fabricated decision. Nothing in the extractor distinguished a
  question about a choice from a statement of one.
\item \textbf{A closed technology whitelist.} Bare tech-name
  matching required the name to appear on a fixed list of
  roughly 50 tools. Real names outside it --- \texttt{sqlc},
  \texttt{NATS}, \texttt{golangci-lint}, \texttt{OpenTelemetry},
  \texttt{Alembic}, among others --- were invisible regardless
  of how explicitly they were stated as a choice.
\item \textbf{Narrow trigger-verb vocabulary.} Phrasings such as
  ``We're \emph{leaning toward} NATS'' used a verb absent from
  both the explicit-verb pattern and the tech-name context
  check.
\item \textbf{Error vocabulary and truncated context.} ``nil
  pointer dereference'' (Go's idiom) was not recognised ---
  only ``null pointer'' was --- and terms including
  \emph{GC pressure}, \emph{poison message}, \emph{consumer
  lag}, and \emph{schema drift} were entirely absent from the
  symptom vocabulary. Separately, the trailing-context capture
  extended a label only after the prepositions \emph{in}/
  \emph{on}/\emph{from}, so ``deadlock \emph{between} the two
  mutexes'' and ``poison message \emph{halting} the consumer
  group'' were truncated to the bare symptom noun --- correctly
  identified, but short enough of the full phrase to fall below
  the $\theta=0.6$ coverage threshold (Definition~\ref{def:coverage})
  against the complete ground-truth label.
\end{enumerate}

\subsection{A Fix That Was Tried and Rejected}
\label{sec:rejected-fix}

Cause 2 is an open-vocabulary problem: any fixed list of tool
names will always lag real ones. The natural-looking fix is a
whitelist-free fallback matching tool-\emph{shaped} tokens ---
CamelCase (\texttt{OpenTelemetry}), kebab-case
(\texttt{golangci-lint}), short ALLCAPS acronyms --- gated by
the same decision-context check already used for the whitelist.
This was implemented and measured, not merely reasoned about:
on the product repository's own 14-session eval corpus
(the corpus its CI quality gate is scored against, disjoint
from this study's 100-session corpus), it raised decision
recall but dropped decision precision from $90\%$ to $81\%$,
even restricted to the CamelCase case alone. The cause is that
ordinary English produces the same shapes for non-tool words:
kebab-case catches ``per-row'' and ``pre-download''; ALLCAPS
catches ``CSS'' and ``DMS''; CamelCase mentions of real but
contextually irrelevant libraries (\texttt{PyTorch},
\texttt{HuggingFace}) also fired. Shape alone is not a
sufficiently reliable proxy for ``tool name'', so this fix was
reverted; a bounded, verified whitelist expansion was used for
cause 2 instead. This negative result is reported because a
paper that shows only the fixes that worked would overstate how
easy the open-vocabulary problem is, and because the reverted
code's rationale is preserved as an inline comment in the
extractor specifically so the same attempt is not repeated
identically by a future contributor without first reading why
it failed.

\subsection{Fixes Applied and Measured Effect}

Causes 1, 3, and 4 were fixed directly: a question-context guard
(skip a match whose sentence ends in \emph{?}) now gates every
decision pass; the verb and tech-name context vocabularies gained
\emph{leaning toward} and \emph{recommend(ed)}; the error symptom
vocabulary gained \emph{nil pointer dereference}, \emph{GC
pressure}, \emph{poison message}, \emph{consumer lag},
\emph{schema drift}, and \emph{partition skew}, and its
trailing-context capture now also continues after
\emph{between}/\emph{during}/\emph{under}/\emph{while}/\emph{across}
and a small set of gerund verbs (\emph{halting}, \emph{causing},
\emph{growing}, \ldots). Cause 2 was addressed by adding eight
specific tool names verified missing from this study's corpus
(\texttt{sqlc}, \texttt{dbt}, \texttt{NATS}, \texttt{pnpm},
\texttt{uv}, \texttt{ruff}, \texttt{kong}, \texttt{airflow}) to
the existing whitelist, after the shape-based alternative in
Section~\ref{sec:rejected-fix} was rejected.

Every fix was checked against the product's own 610-test suite
(all passing, no regression) and the 14-session eval corpus
(decision precision/recall unchanged at 90\%/95\%, confirming
the fixes closed the n=100 gap without trading away quality on
the corpus the product's CI is actually gated on) before being
measured on this study's 100-session corpus.
Table~\ref{tab:fix-cycle} reports the result: releasing as
TokenMizer 0.5.3.

\begin{table}[t]
  \centering
  \caption{Fix-and-Remeasure Cycle, n=100. Same corpus,
  threshold, and bootstrap configuration before and after.}
  \label{tab:fix-cycle}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{@{}lrrr@{}}
    \toprule
    \textbf{Metric} & \textbf{0.5.2} & \textbf{0.5.3} & \textbf{$\Delta$} \\
    \midrule
    Macro F1            & 57\% & \textbf{60\%} & $+3$pp \\
    Decision F1         & 50\% & \textbf{59\%} & $+9$pp \\
    Error F1            & 36\% & \textbf{44\%} & $+8$pp \\
    Completed-task F1   & 68\% & 68\% & $+0$pp \\
    File F1             & 98\% & 98\% & $+0$pp \\
    Implicit-register F1 & 25\% & 27\% & $+2$pp \\
    vs.\ Graphiti-style ($\Delta$) & $-2.7$pp & $+0.4$pp & --- \\
    vs.\ Mem0-style ($\Delta$)     & $-2.8$pp & $+0.3$pp & --- \\
    Resume tokens       & 99 & 100 & $+1$ \\
    \bottomrule
  \end{tabular}
\end{table}

The categories the fixes targeted moved (decisions, errors);
the categories they did not touch stayed flat (completed tasks,
files), which is the expected signature of a targeted fix
rather than a corpus-fitting artefact that would be expected to
move everything at once. The implicit-register score moved only
2 points --- inside noise at this sample size --- confirming
directly what Section~\ref{sec:extended-register} argues
structurally: vocabulary and context-window fixes reach
explicit and semi-explicit phrasing, not the register ceiling
itself, which is a different problem requiring a different
solution (Section~\ref{sec:llm-extractor}).
...
interval on the paired difference crosses zero in both the
0.5.2 and 0.5.3 measurements (Table~\ref{tab:fix-cycle}) ---
the fix cycle moved the point estimate from trailing to
nominally leading, but did not turn the tie into a reliable
win, and it should not be reported as one. Graphiti's
...
(Table~\ref{tab:category}) were TokenMizer's most improvable
categories in absolute terms and relative to the peer group
simultaneously, which was a stronger signal than either fact
alone: a category that is merely low but consistent with peers
suggests a corpus-difficulty explanation; a category that is
low \emph{and} behind peers scored on the identical corpus
points at the extraction patterns themselves. Section~\ref{sec:fix-cycle}
tested that reasoning directly rather than leaving it as
inference: tracing the actual missed and spurious items to four
specific causes and fixing three of them closed roughly a third
of the decision gap and a fifth of the error gap to
Graphiti-style/Mem0-style, which is evidence the diagnosis was
substantially right, not merely plausible. The remaining gap in
both categories, and the full gap on implicit-register text
(Table~\ref{tab:fix-cycle}'s near-zero movement there), is
evidence of what vocabulary and context-window fixes cannot
reach without a model actually reading the sentence.
...
blocks, and reports two evaluations of it plus one complete
fix-and-remeasure cycle between them.
...
--- replicated that advantage with formal statistical support
and additionally found, in its first pass (TokenMizer 0.5.2),
that the advantage did not extend to two comparably-designed
memory strategies (Graphiti-style, Mem0-style), where the
honest result was a tie, and that decision and error extraction
were TokenMizer's most improvable categories both in absolute
terms and relative to those two peers. Tracing the actual
missed and spurious items behind that finding to four specific
causes in the extraction code, fixing three of them directly
and rejecting a fourth after it measurably hurt precision on an
unrelated corpus, and re-running the identical benchmark
(TokenMizer 0.5.3) moved the point estimate from trailing both
peers to nominally leading both, while the honest statistical
reading stayed a tie throughout --- the fix closed roughly a
third of the decision gap and a fifth of the error gap without
changing the paper's central conclusion, which is itself
evidence that conclusion was measuring something real rather
than an artefact of one extraction version. Every pattern-matching
method tested, independent of implementation and unmoved by the
fix cycle, shares a common ceiling of roughly 20\% macro F1 on
text that states facts indirectly.
...
which Section~\ref{sec:fix-cycle} showed directly that a
vocabulary fix does not, rather than leaving it as an inferred
