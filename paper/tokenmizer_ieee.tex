...
%
%  Single-version manuscript. Evaluates TokenMizer 0.5.3 (the
...
Manuscript received June 4, 2026; revised August 12, 2026.}%
}
...
We evaluate TokenMizer 0.5.3 --- the released product, run
...
TokenMizer scores $57\%$ macro F1 ($95\%$ CI $[52\%,62\%]$),
...
CI on the difference crosses zero for both).
TokenMizer's resume block (99 tokens) is smaller than
...
extraction ($50\%$ F1, against $65\%$ for Graphiti/Mem0-style)
and error extraction ($36\%$ F1, against $66\%$).
...
(Section~\ref{sec:extended}): 176\,ms per session end-to-end,
...
TokenMizer 0.5.3 is implemented in Python 3.10+, comprising
...
0.5.3, invoked out-of-process against a real checkout so that
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrr@{}}
    ...
    \textbf{Tok.} & \textbf{ms} \\
    \midrule
    Mem0-style       & 60\% & [55, 64] & 118 & 1.0 \\
    Graphiti-style   & 59\% & [55, 64] & 120 & 1.0 \\
    \rowcolor{rowours}
    \textbf{TokenMizer 0.5.3} & \textbf{57\%} & [52, 62] & 99 & 176.0 \\
    GraphRAG-style   & 44\% & [41, 48] & 80  & 0.6 \\
...
against $66\%$ for the same two).
Of 355 labelled decisions, TokenMizer extracts 274 (recall
$45\%$), of which 118 do not match any ground-truth item
(precision $57\%$); of 148 labelled errors, it extracts 79
(recall $28\%$), of which 39 do not match
(precision $51\%$). Both categories are simultaneously
...
the extraction patterns themselves --- the clearest, most
concrete target for engineering improvement this study
identifies.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \midrule
    \rowcolor{rowours}
    TokenMizer      & \textbf{68\%} & \textbf{53\%} & 50\% & \textbf{98\%} & 36\% \\
...
TokenMizer's mean 176.0\,ms per session is 170--440$\times$ the
...
simultaneously. The per-session results released alongside this
manuscript (Section~\ref{sec:availability}) make the specific
missed and spurious items behind this finding directly
inspectable --- the natural next step this paper stops short of
is tracing those items back to specific causes in the extraction
patterns, the way Section~\ref{sec:extended-register} does
structurally for the register ceiling, and we leave that
targeted engineering analysis as the clearest piece of concrete
future work this study identifies rather than attempting it
post hoc within the scope of a single measurement study.
...
blocks, and evaluates the released 0.5.3 product on a
...
a loss or a win.
Decision and error extraction are TokenMizer's most improvable
categories, both in absolute terms and relative to the two
peer methods it otherwise ties. Every pattern-matching method
tested, independent of implementation, shares a common ceiling
of roughly 20\% macro F1 on text that states facts indirectly.

Three directions follow directly from these results.
\textbf{First}, the released per-session results
(Section~\ref{sec:availability}) support a targeted error
analysis of the decision and error categories identified in
Section~\ref{sec:discussion} --- tracing specific missed and
spurious items to specific causes in the extraction patterns,
rather than treating ``needs improvement'' as the actionable
conclusion.
...
MIT licence; this manuscript evaluates release 0.5.3.
