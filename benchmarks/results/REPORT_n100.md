...
**Date:** August 12, 2026
**Data:** 100 labelled sessions, 8 methods compared, TokenMizer version 0.5.3
**Files:** `benchmarks/results/memorybench_n100_20260812.json` (full numbers), `.csv` (spreadsheet-friendly), `dashboard.html` (visual view)
...
We tested TokenMizer (the real product, version 0.5.3) against 7 other memory methods on 100 test sessions. Each method reads a conversation and tries to pull out 5 things: completed tasks, pending tasks, decisions, file names, and errors. We compared what each method found against a hand-checked answer key.

**Main result:** TokenMizer clearly beats simple methods (keeping the last few messages, summarizing, etc.) and two other graph-based methods (MemGPT-style and GraphRAG-style). Against two other methods — Graphiti-style and Mem0-style — the difference is small enough that we cannot call it a real win. It's a statistical tie.

**Main weakness found:** TokenMizer is noticeably worse than Graphiti-style and Mem0-style at finding **decisions** and **errors** in a conversation. This is the clearest place to improve, and the released per-session data (Section 9) makes it possible to trace exactly which sessions and which items drive that gap.

**Also found:** every method we tested, including TokenMizer, does much worse when a conversation states things indirectly instead of using clear markers like "Decided:" or "Completed:". This is not a TokenMizer-only problem — all 4 pattern-based methods dropped to about 19–25% accuracy on indirect text.
...
|---|---:|---:|---:|---:|
| **TokenMizer 0.5.3** | **57%** | 52%–62% | 99 | 176 ms |
| Graphiti-style | 59% | 55%–64% | 120 | 1.0 ms |
| Mem0-style | 60% | 55%–64% | 118 | 1.0 ms |
| GraphRAG-style | 44% | 41%–48% | 80 | 0.6 ms |
...
| GraphRAG-style | +13 points | **Yes, TokenMizer wins** |
...
**Decisions and errors are the weak spots.** Of 355 decisions mentioned across all 100 sessions, TokenMizer found 274, but 118 of those didn't actually match a real decision (a lot of wrong guesses alongside the right ones). For errors: out of 148 mentioned, TokenMizer found 79, and 39 of those were wrong. This is the single clearest thing to fix, and it's traceable — the released per-conversation results (Section 9) show exactly which sessions and which specific missed/wrong items are behind these numbers.
...
Every number in this report comes from `benchmarks/results/memorybench_n100_20260812.csv` — one row per conversation, per method, per category, with the exact counts (expected / found / correct / wrong) behind every percentage above. To find exactly *which* decisions or errors TokenMizer missed or got wrong (the natural next step after Section 3), filter that CSV to `method=tokenmizer_v0.5.2` (the harness's internal key for the TokenMizer row; the report and dashboard both label it "TokenMizer 0.5.3", the actual product version) and `category=decisions` or `category=errors`, then cross-reference the session IDs against `benchmarks/corpus/*.json` to read the actual conversation text.

That kind of item-by-item tracing is how a future update to this benchmark would turn "decisions and errors are weak" into a specific, fixable list of causes — the same way Section 4's register finding was confirmed structurally rather than just observed in aggregate. It is out of scope for this report, which measures where TokenMizer 0.5.3 stands today, not why.
